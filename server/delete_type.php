...
if (isset($_POST['typeid']) && !empty($_POST['typeid'])) {
    ...
    try {
        ...
        if ($type) {
            ...
            header('Location: ../pages/settings.php?section=type&success=Type deleted successfully');
            exit();
        } else {
            // Type does not exist
        ...
        // Handle any errors
        header('Location: ../pages/settings.php?section=type&error=Failed to delete type: ' . $e->getMessage());
        exit();
    }
} else {
    // Type ID is missing or invalid
    header('Location: ../pages/settings.php?section=type&error=Invalid type ID');
    exit();
}
?>
